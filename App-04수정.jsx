...
function App(){
    ...
    const memberEdit = async (p_collection, p_id, p_pass, p_name) => {
        //기존 입력 함수와 완전히 동일함. 즉 기존 도큐먼트가 있으면 수정 처리된다.
    ...
    //select태그의 내용을 추가하기 위한 스테이트트
    const[collName, setCollName] = useState([]);
    
    //화면의 렌더링이 끝난 후 실행되는 수명주기 함수수
    useEffect(() => {
    ...
    //함수 호출후 콜백 데이터를 then절에서 처리 
        getCollection().소두((result) => {
    ...
    /*
        useEffect의 두번째 인자인 의존성배열은 빈배열을 적용하여 렌더링 후 딱 한번만 실행되도록 처리한다.
    */
    return(
        <div className="App">
            ...
            */}
            <select onChange={async (e)=>{
                //선택 항목의 value를 변수에 지정한다. 즉 아이디를 저장한다.
